...
module Rnexus
  class Plotter
    ...
    def get_one_day(day_string, type = :T0)
      ...
      data = Rnexus::DBManager::Measurement.where(:DT => start_timestamp..end_timestamp).order(:DT).map {|m| [dt_str(m.DT),m.send(type.to_sym)]}
      return data
    end
    ...
    def get_days_min_max(cnt_days = 7, type = :T0)
      ...
      data = Rnexus::DBManager::Measurement.where(:DT => start_timestamp..end_timestamp).order(:DT).map {|m| [m.DT, m.send(type.to_sym)]}
      days = {}
      data.each do |d|
